...
Sine Transform of two-dimensional real array using FFTW libraries
...
Compile your program with g++ and proper path to FFTW library, for example in Ubuntu:
    g++ tcdcastufftwl.2d.64.cpp -L/usr/local/bin -lfftw3
  If you instead wish to compile using gcc:
    gcc tcdcastufftwl.2d.64.cpp -L/usr/local/bin -lfftw3 -lstdc++ -lm
...
int main()
{
    ...
    int n1 = 7;
    double *in2cs, *out2cs;
    in2cs = (double*) fftw_malloc(sizeof(double) * n0 * n1);
    out2cs = (double*) fftw_malloc(sizeof(double) * n0 * n1);
    fftw_plan p2cs;
    p2cs = fftw_plan_r2r_2d(n0, n1, in2cs, out2cs, FFTW_REDFT10, FFTW_REDFT10, FFTW_ESTIMATE);

// Define input here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<n1;i1++) {
            in2cs[i0*n1+i1]=i0*i1*1.0;
        }
    }

    fftw_execute(p2cs);
    fftw_destroy_plan(p2cs);
    
// Use output here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<n1;i1++) {
            cout<<out2cs[i0*n1+i1]<<endl;
        }
    }
    
// If not required, deallocate input and output arrays
    fftw_free(in2cs); fftw_free(out2cs);
