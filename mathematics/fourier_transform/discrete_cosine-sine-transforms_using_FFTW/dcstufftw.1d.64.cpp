...
Sine Transform of one-dimensional real array using FFTW libraries
...
Define Transform type in plan as:
...
Compile your program with g++ and proper path to FFTW library, for example in Ubuntu:
    g++ tcdcastufftwl.1d.64.cpp -L/usr/local/bin -lfftw3
  If you instead wish to compile using gcc:
    gcc tcdcastufftwl.1d.64.cpp -L/usr/local/bin -lfftw3 -lstdc++ -lm
...
For input data X_0, X_1, ... , X_(n-1), the transform Y_k (for k=0,n-1)
...
int main()
{
    ...
    int n0 = 15;
    double *in1cs, *out1cs;
    in1cs = (double*) fftw_malloc(sizeof(double) * n0);
    out1cs = (double*) fftw_malloc(sizeof(double) * n0);
    fftw_plan p1cs;
    p1cs = fftw_plan_r2r_1d(n0, in1cs, out1cs, FFTW_REDFT10, FFTW_ESTIMATE);
...
// If not required, deallocate input and output arrays
    fftw_free(in1cs); fftw_free(out1cs);
