...
of two-dimensional complex array to real array using FFTW libraries
...
Define values of n0 and n1 as lengths of array, where
  input array fftw_complex[n0*(1+n1/2)], output array double[n0*n1]
...
Compile your program with g++ and proper path to FFTW library, for example in Ubuntu:
    g++ tc2dfftufftwl.c2r.64.cpp -L/usr/local/bin -lfftw3
  If you instead wish to compile using gcc:
    gcc tc2dfftufftwl.c2r.64.cpp -L/usr/local/bin -lfftw3 -lstdc++ -lm

NOTE:
c2r is an inverse transform; for forward transform, use r2c
In case of c2r, the input is fftw_complex array of size n0*(1+n1/2) and
  output is double array of size n0*n1
...
Arrays are stored as one-dimensional arrays in row-major format and 
  (i0,i1) element of input can be accessed via (i0*(1+n1/2)+i1) element
  (i0,i1) element of output can be accessed via (i0*n1+i1) element
Storing only 1+n1/2 columns of input is done because (n1-1)th column is complex
  conjugate of 1st (not 0th, as per c++ convention), (n1-2)th is complex
  conjugate of 2nd and so on..
Default c2r transform for 2d changes the input array
*/
...
int main()
{
    ...
    fftw_complex *in2c2r;
    double *out2c2r;
    in2c2r = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * n0 * (1+n1/2));
    out2c2r = (double*) fftw_malloc(sizeof(double) * n0 * n1);
    fftw_plan p2c2r;
    p2c2r = fftw_plan_dft_c2r_2d(n0, n1, in2c2r, out2c2r, FFTW_ESTIMATE);

// Define input here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<(1+n1/2);i1++) {
            in2c2r[i0*(1+n1/2)+i1][0]=i0*i1*1.0; in2c2r[i0*(1+n1/2)+i1][1]=i0*i1*0.5;
        }
    }

    fftw_execute(p2c2r);
    fftw_destroy_plan(p2c2r);
    
// Use output here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<n1;i1++) {
            cout<<out2c2r[i0*n1+i1]<<endl;
        }
    }
    
// If not required, deallocate input and output arrays
    fftw_free(in2c2r); fftw_free(out2c2r);
