...
of three-dimensional real array using FFTW libraries
...
Define values of n0, n1 and n2 as lengths of array, where
  input array double[n0*n1*n2], output array fftw_complex[n0*n1*(1+n2/2)]
...
Compile your program with g++ and proper path to FFTW library, for example in Ubuntu:
    g++ tc3dfftufftwl.r2c.64.cpp -L/usr/local/bin -lfftw3
  If you instead wish to compile using gcc:
    gcc tc3dfftufftwl.r2c.64.cpp -L/usr/local/bin -lfftw3 -lstdc++ -lm

NOTE:
r2c is a forward transform; for inverse transform, use c2r
In case of r2c, the input is double array of size n0*n1*n2 and output is
  fftw_complex array of size n0*n1*(1+n2/2)
...
Input and Output arrays can be same, in which case the input array
  ...
  of sufficient size, i.e. n0*n1*2*(1+n1/2)
FFTW computes unnormalized FFTs
Arrays are stored as one-dimensional arrays in row-major format and 
  (i0,i1,i2) element of input can be accessed via ((i0*n1+i1)*n2+i2) element
  (i0,i1,i2) element of output can be accessed via ((i0*n1+i1)*(1+n2/2)+i2) element
Storing only 1+n2/2 entries in last dimension of output is done because (n2-1)th entry
  is complex conjugate of 1st (not 0th, as per c++ convention), (n2-2)th is complex
...
int main()
{
    ...
    int n2 = 3;
    double *in3r2c;
    fftw_complex *out3r2c;
    in3r2c = (double*) fftw_malloc(sizeof(double) * n0 * n1 * n2);
    out3r2c = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * n0 * n1 * (1+n2/2));
    fftw_plan p3r2c;
    p3r2c = fftw_plan_dft_r2c_3d(n0, n1, n2, in3r2c, out3r2c, FFTW_ESTIMATE);

// Define input here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<n1;i1++) {
            for(int i2=0;i2<n2;i2++) {
                in3r2c[(i0*n1+i1)*n2+i2]=i0*i1*i2*1.0;
            }
        }
    }

    fftw_execute(p3r2c);
    fftw_destroy_plan(p3r2c);
    
// Use output here
    for(int i0=0;i0<n0;i0++) {
        for(int i1=0;i1<n1;i1++) {
            for(int i2=0;i2<(1+n2/2);i2++) {
                cout<<out3r2c[(i0*n1+i1)*(1+n2/2)+i2][0]<<"  "<<out3r2c[(i0*n1+i1)*(1+n2/2)+i2][1]<<endl;
            }
        }
    }
    
// If not required, deallocate input and output arrays
    fftw_free(in3r2c); fftw_free(out3r2c);
